...
class SixSuite extends munit.ScalaCheckSuite {
  ...
  property("ipv4 succeeds for all IP addresses") {
    forAll(Generators.ipv4) { ip =>
      assertEquals(parseOnly (Parser.ipv4) (ip.show), Some(ip))
    }
  }
